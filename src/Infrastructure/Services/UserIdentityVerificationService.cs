...
{
    private readonly ReadOnlyCollection<string> _identityLabels = new(new[] {"Id Cards", "Passport", "Driving License"});
    ...
    public UserIdentityVerificationService(IAmazonRekognition amazonRekognition, IOptionsSnapshot<VerificationS3Settings> verificationS3SettingsOptions, IUserRepository userRepository)
    {
    ...
    public async Task<(bool, List<string>)> CheckIsValidIdentityAsync(string idCardUrl, CancellationToken cancellationToken)
    {
        ...
        var labels = response.Labels.Where(q => q.Confidence > _verificationS3SettingsOptions.Value.MinConfidence).Select(x => x.Name).ToList();
        var isIdentity = labels.Any(x => _identityLabels.Contains(x));
        return (isIdentity, response.Labels.Select(q => q.Name).ToList());
    }

    public async Task<bool> CompareFaceAndIdCardAsync(string userId, string faceUrl, string idCardUrl, CancellationToken cancellationToken)
    {
